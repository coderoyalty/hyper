...
namespace lua_bindings
{
  void bindShaderClasses(sol::state &lua)
  {
    ...
    wow.new_enum<SHADER_TYPE>(
        "ShaderType",
        {{"VERTEX", SHADER_TYPE::VERTEX}, {"FRAGMENT", SHADER_TYPE::FRAGMENT}});

    // shader
    sol::usertype<Shader>
        shader_type = wow.new_usertype<Shader>("Shader", sol::constructors<Shader(SHADER_TYPE)>());
    ...
    // shader program
    sol::usertype<ShaderProgram> program_type = wow.new_usertype<ShaderProgram>("ShaderProgram", sol::constructors<ShaderProgram()>());
